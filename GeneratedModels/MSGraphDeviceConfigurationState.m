...
@implementation MSGraphDeviceConfigurationState

- (id) init
{
    if (self = [super init]) {
        self.oDataType = @"#microsoft.graph.deviceConfigurationState";
    }
    return self;
}
- (NSArray*) settingStates
{
    ...
    if ([settingStates isKindOfClass:[NSArray class]]){
        for (id tempDeviceConfigurationSettingState in settingStates){
            [settingStatesResult addObject:tempDeviceConfigurationSettingState];
        }
    }
